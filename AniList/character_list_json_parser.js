...
function renderCharacters(charList, limit, selectedLang) {
  return charList.slice(0, limit).map(char => {
    ...
    return `
    <figure class="character grid grid-cols-[60px_1fr_1fr_60px] gap-2.5 items-center bg-[rgb(var(--color-foreground))] rounded overflow-hidden text-xs">
  <img src="${char.img}" alt="${char.name}" class="a1"/>
  <figcaption class="contents">
    <p class="a2">${char.name}</p>
    <p class="a3">${char.role}</p>
    <div class="a4 voice-actors">
      ${vas.length > 0 ? vas.map(va => `
        <div class="voice-actor grid grid-cols-[1fr_60px] gap-1">
          <div class="va-caption grid items-center">
            <p class="va_name"><strong>${va.name}</strong></p>
            <p class="va_language">${va.language}</p>
          </div>
          <img src="${va.img}" alt="${va.name}"/>
        </div>
...
function renderTab(tab, data, selectedLang = "JAPANESE") {
  if (tab === 'overview') {
    return `
      <h2>Characters</h2>
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-5">${renderCharacters(data.characters, 6, selectedLang)}</div>
      <h2>Staff</h2>
  ...
  if (tab === 'staff') {
    return `
      <h2>Staff</h2>
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-5">${renderStaff(data.staff, data.staff.length)}</div>
    `;
  }
  if (tab === 'characters') {
    return `
      ...
      <h2>Characters</h2>
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-5">${renderCharacters(data.characters, data.characters.length, selectedLang)}</div>
    `;
  }
  return '';
}
